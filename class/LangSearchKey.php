...
class LangSearchKey extends ControlPanel
{
	protected $arrConfig = array(
				'title'=> '本地化翻译',
				'view'=>array(
					'template'=>'LangSearchKey.html',
					'class'=>'view',
	...
	{
		$sSearchLangCountry = $this->params['sLangCountry'];
		$sSearchKey = $this->params['searchkey'];
		$this->view->variables()->set('hidelangcountry', $sSearchLangCountry);
		if(empty($sSearchKey))
		{
...
// 		$arrLangCountry = explode('_',$sSearchLangCountry);
...
// 		}
		...
		if($this->params['paginator'])
		{
			...
			$sLangCountry = $this->params['sLangCountry'];
			$sSearchKey = $this->params['searchkey'];
			
			$arrSentenceLibrary = $this->getSelectSentenceLibrary($sLangCountry);
			$arrSentenceLibrarySeach = $this->searchKey($arrSentenceLibrary,$sSearchKey);
			$arrLangTranslationSelect = $this->setSelectSentenceLibraryPage('', $arrSentenceLibrarySeach);
			$arrLangTranslationChunk = array();
			$arrLangTranslationChunk = $this->getLangChunk($arrLangTranslationSelect,$nPerPageRowNumber=20);
			$arrLangTranslationNew = $this->getSelectSentenceLibraryNew($sSearchKey, $arrLangTranslationChunk,$iCurrentPageNum);
			$this->view->variables()->set('hidelangcountry', $sLangCountry);
			$this->view->variables()->set('arrLangTranslation',$arrLangTranslationNew);
		}
	...
	public function form()
	{
		...
		$i=0;
		$sLangCountry = $this->params['hidden_lang_country'];
		...
		$this->createMessage(Message::success,"%s ",$skey="保存成功") ;
		$this->location('?c=org.opencomb.localizer.LangTranslation&sSwichFrontLangPath='.$sLangCountry);
